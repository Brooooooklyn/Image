...
if (!nativeBinding || forceWasi) {
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === 'wasm32-wasi')) {
    ...
    try {
      candidateError = __napiWasiResolveCandidate('./image.wasi.cjs', false, ['./image.wasm32-wasi.debug.wasm', './image.wasm32-wasi.wasm'])
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === 'wasm32-wasi')) {
    ...
    let candidateFailed = false
    try {
      candidateError = __napiWasiResolveCandidate('@napi-rs/image-wasm32-wasi', true, undefined)
      candidateFailed = candidateError !== null
      if (!candidateFailed) {
        if (process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
          const bindingPackageVersion = require('@napi-rs/image-wasm32-wasi/package.json').version
          if (bindingPackageVersion !== '1.14.0') {
            throw new Error(`WASI binding package version mismatch, expected 1.14.0 but got ${bindingPackageVersion}. You can reinstall dependencies to fix this issue.`)
          }
        }
